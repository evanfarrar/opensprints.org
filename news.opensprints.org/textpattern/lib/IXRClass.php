...
$HeadURL: https://textpattern.googlecode.com/svn/releases/4.0.8/source/textpattern/lib/IXRClass.php $
...
class IXR_Client {
    ...
    function query() {
        ...
        $fp = (!is_disabled('fsockopen')) ? fsockopen($this->server, $this->port, $errno, $errstr, 45) : false;
		ob_end_clean();
        if (!$fp) {
...
class IXR_Date {
    ...
    function parseIso($iso) {
        ...
        $this->tz = substr($iso, 17);
    }
    function getIso() {
        return $this->year.$this->month.$this->day.'T'.$this->hour.':'.$this->minute.':'.$this->second.$this->tz;
    }
    function getXml() {
